...
def run_on_csv(uploaded_file,thread_number,n_threads):
    ...
    # line_text = st.empty()
    ts = str(timestring())
    outfile_audio = "audio_output_"+audio_csv_file+"_.csv"
    outfile_errors = "audio_output_"+audio_csv_file+"_error.csv"
    ...
    #star_rating = read_variable_from_csv(audio_csv_file,"Star Rating")
    ...
    for i in range((step)*chunk,(step+1)*chunk):
        try:
            ...
            if(response_object):
                ...
                for transcript in list_transcript:
                    ...
                    if(probability>0.4 and probability<0.6):
                        ...
                        if_any = "intermediate"
                    df = pd.DataFrame([[astrologer_idx,id1x,type1x,user_idx,created_atx,order_item_idx,number,url_base,transcript,first_word_time11,confidence,eng_from_hindi,sexual,sexual_score,hate,hate_score,harrasment,harrasment_score,self_harm,self_harm_score,sexual_minor,sexual_minor_score,threatening,threatening_score,violence_graphic,violence_graphic_score,self_harm_intent,self_harm_intent_score,self_harm_instructions,self_harm_instructions_score,violence,violence_score,contact_details,probability,silence,silence_rangex,greeting_response,greeting_prob,if_any]],
                        columns=['astrologer_id','id1','type1','user_id','created_at','order_item_id','number','url_base','transcript','first_word_time','confidence','eng_from_hindi','sexual','sexual_score','hate','hate_score','harrasment','harrasment_score','self_harm','self_harm_score','sexual_minor','sexual_minor_score','threatening','threatening_score','violence_graphic','violence_graphic_score','self_harm_intent','self_harm_intent_score','self_harm_instructions','self_harm_instructions_score','violence','violence_score','contact_details','probability','silence','silence_rangex','greeting_response','greeting_prob','if_any'])
                    #print(df)
                    
                    hdr = False  if os.path.isfile(outfile_audio) else True
                    df.to_csv(outfile_audio, mode='a', header=hdr,sep=',')
            ...
            # Set maximum column width for display (optional)
...
######once download is complete run this ######


def audio_thread(csv_name=str):
    n_threads = 10

    thread_list =[]
    for thr in range(n_threads):
        thread = threading.Thread(target=run_on_csv, args=(csv_name,thr,n_threads),)
