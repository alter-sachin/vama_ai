...
def run_on_chat_csv(uploaded_file,n_threads):
    AUDIO_URL = {
    ...
    ts = str(timestring())
    outfile_chat = "chat_result_"+ts+".csv"
    outfile_errors = "chat_error_"+ts+".csv"
    ...
    for i in range(1,total_urls):
        try:
            percentage = int((i/total_urls))
            print("pis",percentage)
            # progress_bar.progress(percentage)
            print("Processing line number %i" % i +" out of total lines %i" %total_urls)
            print("Percentage Completed %i" % percentage + "%")
            from_idx = from_id[i]
            ...
            df.to_csv(outfile_chat, mode='a', header=hdr)
...
for thr in range(n_threads):
    thread = threading.Thread(target=run_on_chat_csv, args=("1_agora.csv",n_threads),)
